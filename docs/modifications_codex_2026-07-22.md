...
- Tables SQLite : `bot_state`, `bot_positions`, `bot_trailing_stops`, `cryptos`, `ml_exit_recommendations`, `decision_logs`, `ml_feature_values`, `ml_trade_outcomes`, `ml_open_entries`, `notifications`, `support_touch_results`, `ml_model_metadata`, `ml_feature_importances`, `ml_analysis_runs`, `ml_prediction_calibration`, `ml_rejected_replay_results`, `ml_drift_alerts`.
...
- Etat trading recree proprement : `bot_state` garde une ligne par mode avec colonnes pour les scalaires. `bot_positions` garde les positions et les ordres sell ouverts, afin d'éviter une table `pending_orders` redondante. Les recommandations de sortie ML sont isolées dans `ml_exit_recommendations`, et le contexte/predictions live dans `cryptos`.
